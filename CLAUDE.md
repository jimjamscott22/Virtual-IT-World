...
| `web/templates/_queue.html` | Task 16 prose: show "a truncated opening line" | Task 19's `test_a_full_ticket_can_be_worked_through_http` asserts `ticket.report_text in client.get("/").text` — the two tasks' own descriptions conflict. The queue row renders the full `report_text` (`| safe`, same reasoning as `_ticket.html`), not a 60-char slice of `symptoms.opening`. |
