...
`AppSession` still builds with `TemplatePersona` only. Constructing the model-backed persona from `VITSC_PERSONA` / `VITSC_BASE_URL` / `VITSC_MODEL` and surfacing a degraded banner is Task 2.
